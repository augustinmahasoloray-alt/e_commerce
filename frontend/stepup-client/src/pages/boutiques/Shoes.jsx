      ...
      <section className="grid md:grid-cols-2 gap-12 px-4 sm:px-6 md:px-10 lg:px-16 xl:px-24 2xl:px-40 py-10">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-6">
            <span className="font-headline text-3xl">89.99 €</span>
            <span className="text-sm text-muted line-through">109.99 €</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {similaires.map((p) => (
            <div key={p.name} className="group cursor-pointer">
              ...
              <p className="text-sm text-textColor/80 font-medium mt-1">{p.price} €</p>
            </div>
          ))}
        </div>
      </section>
            ...
            {compareItems.map((p) => (
              <div key={p.name} className="flex items-center gap-3 bg-surfaceColor rounded-xl p-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium leading-tight">{p.name}</p>
                  <p className="text-xs text-muted">{p.boutique} · {p.price} €</p>
                </div>
              </div>
            ))}
